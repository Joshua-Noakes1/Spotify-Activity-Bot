...
async function spotify(res, req, iftttData) {
    ...
    if (!existsSync(path.join(__dirname, '../', '../', '../', 'static', 'images', `image-${data.id}.png`))) {
        ...
        if (config.twitter.useTwitter == true) {
            ...
            // attempt to post
            await postTweet({
                "status": `${config.twitter.user != "" ? config.twitter.user : "Local"} played "${data.title != "" ? data.title : ""}" by ${data.artist != "" ? data.artist : ""} on ${currentTime.month} ${currentTime.date}${currentTime.ordinals}, at ${currentTime.time.hour}:${currentTime.time.minutes}${currentTime.time.type}`,
                "media": mediaCard
            });
        }
