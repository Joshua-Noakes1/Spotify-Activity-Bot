...
async function spotify(res, req, iftttData) {
    ...
    if (!existsSync(path.join(__dirname, '../', '../', '../', 'static', 'images', `image-${data.id}.png`))) {
        ...
        if (config.twitter.useTwitter == true) {
            ...
            // regex date time from ifttt
            var iftttDateTime = iftttData.time.match(/(0[1-9]:[0-5][0-9]((\ ){0,1})((AM)|(PM)|(am)|(pm)))|([1-9]:[0-5][0-9]((\ ){0,1})((AM)|(PM)|(am)|(pm)))|(1[0-2]:[0-5][0-9]((\ ){0,1})((AM)|(PM)|(am)|(pm)))/)[0];

            // attempt to post
            await postTweet({
                "status": `${config.twitter.user != "" ? config.twitter.user : "Local"} played "${data.title != "" ? data.title : ""}" by ${data.artist != "" ? data.artist : ""} on ${currentTime.month} ${currentTime.date}${currentTime.ordinals}, at ${iftttDateTime}`,
                "media": mediaCard
            });
        }
