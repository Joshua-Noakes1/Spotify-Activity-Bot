...
async function plexEpisode(res, req, tautulli) {
    ...
    console.log(lcl.blue('[Info]'), `New episode card request (${data.name} - Season ${data.season} Episode ${data.episode} (${data.id}))`);
    ...
    if (!existsSync(path.join(__dirname, '../', '../', '../', 'static', 'images', `image-${data.id}.png`))) {
        ...
        if (data.tmdbID != '000000') {
            ...
            await moviedb.tvInfo({
                ...
                tmdbSuccess = false;
                console.log(lcl.red('[Fail]'), `Failed to grab TMDB info for "${data.name} - Season ${data.season} Episode ${data.episode}"`);
                console.log(err);
            });
        ...
        console.log(lcl.blue('[Info]'), `Image already exists (${data.name} - Season ${data.season} Episode ${data.episode} (${data.id}))`);
