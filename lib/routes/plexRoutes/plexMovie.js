...
async function plexMovie(res, req, tautulli, next) {
    ...
    var data = {
        ...
        "name": tautulli.media.playback.name,
        "tagline": tautulli.media.playback.tagline != '' ? tautulli.media.playback.tagline : tautulli.media.playback.summary,
    ...
    console.log(lcl.blue('[Info]'), `New movie card request (${data.name} (${data.id}))`);

    // check if image exists
    if (!existsSync(path.join(__dirname, '../', '../', '../', 'static', 'images', `image-${data.id}.png`))) {
        // download tautulli poster
        data.images.poster = await downloadImage(data.images.poster !== "" && data.images.poster !== undefined ? data.images.poster : "https://i.imgur.com/W2W3mIV.png", 'Tautulli');
        ...
        if (data.tmdbID != '000000') {
            ...
            await moviedb.movieInfo({
                ...
                tmdbSuccess = false;
                console.log(lcl.red('[Fail]'), `Failed to grab TMDB info for "${data.name}"`);
                console.log(err);
            });
        ...
        if (config.twitter.useTwitter == true) {
            ...
            // attempt to post
            await postTweet({
                "status": `${config.twitter.user != "" ? config.twitter.user : "Local"} started watching ${data.name != "" ? data.name : ""} on ${currentTime.month} ${currentTime.date}${currentTime.ordinals}, at ${currentTime.time.hour}:${currentTime.time.minutes}${currentTime.time.type}`,
                "media": mediaCard
            });
        }
        ...
        console.log(lcl.blue('[Info]'), `Image already exists (${data.name} (${data.id}))`);
