...
async function createImage(imgData, cacheData) {
    ...
    if (imgExist == false) {
        ...
        // load background, resize and blur it to 11
        ...
        // adding country flag to poster
        if (imgData.image.flag != '') {
            console.log(`[Info] Resizing and rounding counrty flag to poster`);
            var countryFlag = await Jimp.read(imgData.image.flag);
            countryFlag = await roundCorners(countryFlag, {
                cornerRadius: {
                    global: 15
                }
            });

            console.log(`[Info] Adding counrty flag to poster`);
            canvasImg.src = await countryFlag.getBufferAsync(Jimp.MIME_PNG);
            ctx.drawImage(canvasImg, width - 125, height - 303)
        }
        ...
        canvasTxt.drawText(ctx, imgData.name, 55, 175, 1160, 100);

        fs.writeFileSync('./image.png', await canvas.toBuffer('image/png'));
    } else {
