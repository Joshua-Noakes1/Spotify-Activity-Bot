...
async function main(data) {
    ...
    try {
        ...
        // add text
        canvasTxt.font = "NotoSansBold,NotoSansJPBold,Helvetica Neue,helvetica,arial,Hiragino Kaku Gothic Pro,Meiryo,MS Gothic,sans-serif";
        canvasTxt.align = "left";
        canvasTxt.fontSize = "65";
        ctx.fillStyle = "#fff";

        // song title
        canvasTxt.drawText(ctx, `${data.title}`, 938, 405, 950, 80);

        // song artist
        if (data.title.length > 30) var artistPos = 530
        else var artistPos = 505;
        canvasTxt.font = "NotoSansMed,NotoSansJPMed,Helvetica Neue,helvetica,arial,Hiragino Kaku Gothic Pro,Meiryo,MS Gothic,sans-serif";
        canvasTxt.fontSize = "43";
        canvasTxt.drawText(ctx, `${data.artist}`, 941, artistPos, 950, 80);
