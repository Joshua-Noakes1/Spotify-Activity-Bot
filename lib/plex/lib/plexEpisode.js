...
async function plexEpisode(res, tautulli, next) {
    ...
    if (imgExist == false) {
        ...
        if (data.tmdb.usingTmdb == true) {
            cacheData.tmdb = {
                "cacheData.tmdb.tmdbID": data.tmdb.tmdbID,
                "cacheData.tmdb.tmdbURL": `https://www.themoviedb.org/tv/${data.tmdb.tmdbID}/season/${data.episode.seasonNumber}/episode/${data.episode.episodeNumber}`
            }
        }
    }
