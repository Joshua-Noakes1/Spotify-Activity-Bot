...
async function plexEpisode(req, res, data) {
    ...
    // get info from tmdb
    var tmdb_episode = await fetch(`https://api.tmdb.org/3/tv/${data.tmdb_id}/season/${data.season}/episode/${data.episode}?api_key=${process.env.TMDBAPIKey}`);
    var tmdb_show = await fetch(`https://api.tmdb.org/3/tv/${data.tmdb_id}?api_key=${process.env.TMDBAPIKey}`);
    ...
    try {
        ...
        if (!tmdb_episode.success) {
            if (!tmdb_show.success) {
                ...
                var backupImages = await loadBackupImages();
            }
        } else {
            var backupImages = await loadBackupImages();
        }
        ...
        function loadBackupImages() {
            ...
            var defaultBackground = fs.readFileSync('./endpoints/plex/types/helper/default.jpeg');
